...
- **Lanzamiento producción:** **`PRODUCTION_LAUNCH_PLAN.md`** + guía ejecutable **`PRODUCTION_ACCOUNTS_SETUP.md`** + plantilla **`docs/env/production.example`**. **D1–D6 cerradas (2026-05-21):** Vercel, Railway, Neon, Auth0, `www.quegym.com`. **Código:** `DATABASE_URL` en partner/leads/analytics (Postgres Neon); fallback SQLite local; `apps/web/vercel.json` para build monorepo. **Sin** Dockerfiles/IaC en repo aún. No cutover DNS hasta GO formal de staging.
- Objetivo recomendado de la próxima sesión: **cuentas + staging** (`staging.quegym.com`) siguiendo `PRODUCTION_ACCOUNTS_SETUP.md`; luego **Sprint 6** (OIDC-only staging + gates + evidencias Sprint 4/5). Merge automático de duplicados y broker outbox quedan en backlog.
