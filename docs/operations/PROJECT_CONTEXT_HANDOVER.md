...
- **Staging desplegado (2026-05):** infra en Neon `quegym`, Railway `quegym-api`, Auth0, Vercel `floit-web`, DNS **`https://staging.quegym.com`**. Registro: **`STAGING_DEPLOYMENT_STATUS.md`**. Paso 2 de `PRODUCTION_ACCOUNTS_SETUP.md` cerrado; **pendiente:** import ~95 venues, smoke/gates, evidencias Sprint 4/5, GO/NO-GO. **Prod `www`:** sin cutover DNS.
- Objetivo recomendado de la próxima sesión: **Paso 3** — `venues:import` staging, fijar URLs Railway en Vercel, `smoke:platform` + `sprint4:gate` + Sprint 5 contra staging; rellenar evidencias. Luego cutover prod si GO.
