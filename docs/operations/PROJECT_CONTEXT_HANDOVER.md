...
- **Staging paso 3 (2026-05-26, parcial):** import **95 venues**; discovery en Vercel OK; URLs 5 servicios documentadas. **Partner Railway:** corregidos en `main` — dep **`express`** y columnas fecha **`timestamptz`** en Postgres (`typeorm-column-types.ts`, 8 entidades; mismo patrón en **leads**). **Pendiente:** confirmar `/health` partner/leads/analytics tras auto-deploy; env Vercel BFF; smoke/gates; **NO-GO** formal. **Prod `www`:** sin cutover DNS.
