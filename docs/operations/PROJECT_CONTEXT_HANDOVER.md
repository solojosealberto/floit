...
- **Staging paso 3 (2026-05-26, parcial):** import **95 venues** en catalog Railway (`venues:import:staging`); Vercel con catalog+search → **`/buscar`**, fichas y comparador OK; URLs públicas de los 5 microservicios documentadas en `docs/env/production.example`. **Pendiente:** `/health` OK en leads, partner (502; partner corregido en repo con dep `express` — redeploy Railway) y analytics (404); variables `LEADS_*` / `PARTNER_*` / `ANALYTICS_*` en Vercel; `smoke:platform` completo; gates Sprint 4/5 y evidencias; **NO-GO** formal hasta entonces. **Prod `www`:** sin cutover DNS.
- Objetivo recomendado de la próxima sesión: redeploy **partner** (y arrancar **leads** / **analytics** en Railway), validar `/health` × 5, completar env Vercel, `pnpm smoke:platform` + gates + evidencias → GO/NO-GO staging.
...
- Login local QA admin por credenciales habilitable vía env (`ADMIN_LOGIN_ALLOW_LOCAL_PASSWORD`, `ADMIN_LOCAL_LOGIN_EMAIL`, `ADMIN_LOCAL_LOGIN_PASSWORD`) con sesión HttpOnly para operación local sin IdP.
