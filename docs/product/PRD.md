...
> Nota de gobernanza: este documento define vision y requisitos de producto. No se usa para tracking vivo de estado de sprint/release. La fuente operativa de estado es `docs/operations/sprints.md`, `docs/operations/EPICS_USER_STORIES_STATUS.md` y `docs/operations/PROJECT_CONTEXT_HANDOVER.md`.
...
#### Criterio operativo adicional para ficha (alineado a implementación actual)

- Cada botón/elemento visible en `/gyms/[slug]` debe tener comportamiento funcional explícito (acción, navegación o estado disabled informado).
- En mobile-first, la jerarquía de acciones debe priorizar:
  1) `Solicitar información`,
  2) `WhatsApp directo`,
  3) `Llamar`/`Email`,
  4) `Guardar`/`Compartir`.
- La instrumentación de eventos en ficha debe mantenerse como requisito de producto (no opcional).
...
## 24. Avance de producto implementado (Partner por centro)

Se incorporó una capacidad clave del PRD para el flujo B2B partner:

- autenticación partner web con sesión segura,
- selección explícita de establecimiento por usuario partner,
- workspace estricto por gym con operaciones scopiadas por `venueSlug`,
- continuidad funcional de configuración de perfil, planes, leads y contenido visual por centro.

Evolución reciente de implementación:

- login partner por `email + contraseña` (OIDC `grant_type=password`) y fallback QA local controlado por entorno para pruebas operativas,
- panel partner con navegación funcional por módulos (`Dashboard`, `Editar perfil`, `Planes y precios`, `Leads recibidos`, `Configuración`),
- actualización de estados de leads desde el panel en modo venue-scoped,
- endurecimiento de operación venue-scoped con endpoint legacy de estado de lead deprecado (`410`),
- mejoras UX en edición partner (selección de `venueSlug` desde perfil y galería/fotos sobre centro activo).

Impacto en hipótesis:

- mejora la trazabilidad y control operativo de cada establecimiento,
- reduce ambigüedad para partners multi-sede,
- prepara el terreno para SLA y analítica por centro en fases siguientes.
