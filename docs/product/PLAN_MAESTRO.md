...
> Nota de gobernanza: este plan es rector de arquitectura y ejecucion, pero no es tracking vivo de estado. La fuente operativa de estado es `docs/operations/sprints.md`, `docs/operations/EPICS_USER_STORIES_STATUS.md` y `docs/operations/PROJECT_CONTEXT_HANDOVER.md`.
...
| **Sprint 6** | Cierre operativo MVP en staging | Cierre US-8.4/US-7.x + validación US-6.x en entorno real | OIDC-only admin/partner en staging, evidencia formal Sprint 4/5, gates con umbrales de staging y decisión GO/NO-GO | `sprint4:gate` + `sprint5:flow-checklist` + `sprint5:kpi-gate` en PASS (staging), evidencia aprobada por tech/product/ops |
| **Sprint 7** | Escalabilidad operativa y brechas parciales | Outbox/eventos + Epic 5 parcial + US-3.4/US-6.4 (priorizado) | Outbox `partner->catalog` con broker externo y worker, observabilidad de colas, workflow editorial de calidad de catálogo, gate de performance en CI | Resiliencia operativa validada, flujo editorial operativo, performance gate activo en PR |
| **Sprint 8** | Fundacion UI y migracion UX usuario (wireframe -> producto) | UX-0, UX-1, UX-2 (discovery) | Taxonomia Figma->Floit, inventario de pantallas, tokens/componentes base en `packages/ui`, rediseño `/buscar` y mapa | No regresion funcional/E2E, consistencia visual base y tracking intacto |
| **Sprint 9** | Migracion UX de conversion y partner | UX-2 (ficha/lead/comparador) + UX-3 | Rediseño `/gyms/[slug]`, `/lead/*`, `/comparar`, `/favoritos`, `/partner/claim`, `/partner/panel`, `/partner/leads` | Funnel y consent sin regresion, experiencia partner usable mobile/desktop |
| **Sprint 10** | Migracion UX admin + cierre QA/rollout | UX-4 + UX-5 | Rediseño `/admin/leads`, `/admin/analytics`, `/admin/partner-claims`, ajuste E2E, validacion KPI y cierre documental | E2E estable, KPI pre/post en rango, rollout gradual con plan de rollback |
...
gantt
    ...
    section Post-MVP inmediato
    Sprint 6 Cierre operativo staging y GO/NO-GO :a7, 2026-07-13, 14d
    Sprint 7 Escala operativa y hardening estructural :a8, 2026-07-27, 14d

    section Migracion UI/UX
    Sprint 8 Fundacion UI y discovery :a9, 2026-08-10, 14d
    Sprint 9 Ficha, lead y partner UX :a10, 2026-08-24, 14d
    Sprint 10 Admin UX, QA y rollout :a11, 2026-09-07, 14d
```

### Plan de ejecución inmediato recomendado (Sprint 6 y 7)

Para preservar alineación con PRD, backlog y plan de prompt engineering, los siguientes dos sprints deben ejecutarse como cierre estructural del MVP y preparación de Release 2, evitando introducir alcance transaccional no validado:

- **Sprint 6 (2 semanas):** enfoque operativo de staging.
  - Activar OIDC-only (`admin` + `partner`) en entorno real.
  - Completar evidencia formal de cierre (`STAGING_EVIDENCE_SPRINT4` y `STAGING_EVIDENCE_SPRINT5`).
  - Ejecutar gates técnicos con umbrales de staging y documentar decisión `GO/NO-GO`.
- **Sprint 7 (2 semanas):** enfoque de robustez y calidad operativa.
  - Conectar outbox `partner->catalog` a broker externo con worker desacoplado.
  - Cerrar brechas parciales de mayor impacto (calidad editorial de catálogo y conversión/feedback).
  - Incorporar gate de performance base a CI para rutas críticas.

### Plan de ejecucion siguiente (Sprint 8, 9 y 10) — migracion UI/UX desde wireframes

Con base en el analisis de `Floit Wireframe v.0.2`, la migracion visual se ejecuta en tres sprints sin alterar contratos ni semantica de negocio:

- **Sprint 8 (2 semanas):** fundacion UI + discovery.
  - Taxonomia y inventario (`FIGMA_TAXONOMY_MAPPING`, `FIGMA_SCREEN_INVENTORY`).
  - Tokens/componentes base en `packages/ui`.
  - Migracion visual de `/buscar` y mapa.
- **Sprint 9 (2 semanas):** conversion usuario + partner.
  - Migracion visual de ficha/lead/comparador/favoritos.
  - Migracion visual de `partner/claim`, `partner/panel`, `partner/leads`.
- **Sprint 10 (2 semanas):** admin + QA + rollout.
  - Migracion visual de paneles admin.
  - Ajustes E2E, validacion KPI pre/post y cierre documental.
...
### Plan funcional inmediato (post-migración visual) — `/gyms/[slug]`

Para consolidar la migración UX en términos de negocio (no solo visual), se ejecuta un mini-plan funcional específico de ficha:

- **P0 conversión:**
  - activar y validar CTAs de contacto (`Solicitar información`, WhatsApp, llamada, email),
  - garantizar continuidad a comparador y envío de lead.
- **P1 utilidades:**
  - `Guardar` (favoritos) y `Compartir` (web share/copy fallback),
  - navegación interna por secciones en mobile-first.
- **P1 contenido contextual:**
  - asegurar bloques de resumen/servicios/planes/ubicación con datos reales o fallback explícito.
- **P0 calidad:**
  - trazabilidad de eventos (`cta_click`, `direct_contact_click`, `lead_submit`, `venue_report`),
  - cumplimiento de checklist visual global en `docs/ux/UI_VISUAL_QA_CHECKLIST.md`.
...
## Actualización de implementación (Partner login + workspace por gym)

Se implementó una base operativa para el modelo `strict single-gym` en partner:

- login partner web por `email + contraseña` (OIDC `grant_type=password`) y sesión por cookie HttpOnly,
- selector de centros `/partner/gyms`,
- acceso al workspace por centro (`/partner/panel/[venueSlug]`),
- endpoints backend y BFF scopiados por `venueSlug` para `profile`, `plans` y `leads`,
- evolución del modelo de `partner_profiles` a `partnerEmail + venueSlug` con fallback/backfill funcional desde perfil global legacy,
- endpoint legacy BFF de estado de lead deprecado (`410`) para forzar operación venue-scoped,
- fallback local QA para pruebas (`seed:ownership`, credenciales partner de prueba y login local controlado por env),
- panel partner habilitado por secciones funcionales (`Dashboard`, `Editar perfil`, `Planes y precios`, `Leads recibidos`, `Configuración`) con acciones reales de estado de leads.

Con esto, la operación partner deja de depender del panel global y queda alineada con la arquitectura por ownership de centro.

---

## Directriz vigente de ejecución (Sprint UI actual)

Para la siguiente iteración inmediata, se define una directriz temporal de priorización:

- **Primero:** cierre de UI/UX de páginas faltantes o no mejoradas (partner/admin/public).
- **Después:** retomar trabajo de hardening operativo (OIDC-only, staging gates, outbox/broker) sin perder trazabilidad.

### Alcance priorizado de páginas (UI)
...
2. **Admin (P1/P2):**
   - `/admin/venues` (P1)
   - `/admin/taxonomias` (P2) — **entregado:** CRUD + API catalog + BFF (ver `docs/operations/EPICS_USER_STORIES_STATUS.md` US-5.2)
   - `/admin/duplicados` (P2)
   - `/admin/moderacion-media` (P2)
3. **Fuera de MVP transaccional (P3):**
   - `/checkout`, `/reservas` (solo placeholder/backlog, sin desarrollo de flujo transaccional real)

### Regla de control de alcance

- Si una solicitud se desvía de este scope UI del sprint, se registra como pendiente y se atiende al cierre de P1/P2.
- El avance se valida con referencia visual y consistencia con componentes del sistema (`@floit/ui`).

Esta directriz no reemplaza la arquitectura objetivo; solo ordena la secuencia de ejecución para terminar el frente de producto visual antes de abrir nuevo alcance.