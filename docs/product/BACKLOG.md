...
**Estado en código:** CRUD admin operativo (`/admin/taxonomias`, API catalog, BFF). Refinamiento pendiente opcional: filtros/discovery que respeten exclusivamente slugs **activos** de la tabla de taxonomía (los venues siguen persistiendo arrays de slugs como hasta ahora).
...
## Actualización de ejecución sobre Epic 4 (partner)

Avance implementado:

- login partner web por `email + contraseña` con sesión HttpOnly (OIDC `grant_type=password`) y fallback QA local controlado por env,
- selector de centros (`/partner/gyms`) y workspace por gym (`/partner/panel/[venueSlug]`),
- operaciones de partner scopiadas por `venueSlug` para perfil, planes y leads en backend + BFF,
- endpoint legacy BFF de estado de lead deprecado con `410` para forzar operación venue-scoped,
...
- UX de edición refinada: selección de `venueSlug` desde perfil y sección de fotos ligada al centro activo.

Este avance fortalece US-4.2, US-4.3 y US-4.4 para escenarios multi-centro y deja base para retirar gradualmente flujos globales legacy.
