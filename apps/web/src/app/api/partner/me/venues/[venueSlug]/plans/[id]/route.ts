...
export async function PATCH(request: Request, { params }: Params) {
  ...
  try {
    const res = await fetch(
      `${base.replace(/\/$/, "")}/v1/partner/me/venues/${encodeURIComponent(venueSlug)}/plans/${encodeURIComponent(id)}`,
      {
        method: "PATCH",
        headers: { [auth.headerName]: auth.headerValue, "content-type": "application/json" },
